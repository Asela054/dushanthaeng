                            ...
                            <div class="col">
                                ...
                                <div class="form-row mb-1">
                                    <div class="col-12">
                                        ...
                                        <input type="text" id="kpi" name="kpi" class="form-control form-control-sm"
                                            required>
                                    </div>
                                </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#create_record').click(function () {
                $('.modal-title').text('Add New Functional KPI');
        ...
        $('#formTitle').on('submit', function(event){
                ...
                if ($('#action').val() == 'Add') {
                    action_url = "{{ route('functionalkpiinsert') }}";
                }
                if ($('#action').val() == 'Edit') {
                    action_url = "{{ route('functionalkpiupdate') }}";
                }
        ...
        $(document).on('click', '.edit', async function () {
                ...
                $.ajax({
                    url: '{!! route("functionalkpiedit") !!}',
                        type: 'POST',
                        dataType: "json",
                        data: {id: id },
                    success: function (data) {
                        $('#type').val(data.result.type_id);
                        $('#kpi').val(data.result.kpi);
                        $('#hidden_id').val(id);
